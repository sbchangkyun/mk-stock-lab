...
check('Selected stock state is separate',
  source.page.includes('chart-stock-header') && source.page.includes('chartAiSelectedSymbol'));
...
process.stdout.write('Stock header and chart-first shell:\n');
check('Central stock header exists', source.page.includes('class="chart-stock-header"'));
for (const id of [
  'chartAiSelectedName', 'chartAiSelectedSymbol', 'chartAiSelectedExchange',
  'chartAiSelectedAssetType', 'chartAiSelectedCurrency',
]) {
  check(`Stock header includes ${id}`, source.page.includes(`id="${id}"`));
}
check('Stock header includes sample label', source.page.includes('샘플 데이터'));
check('Stock header includes non-live label', source.page.includes('실제 시세 아님'));
...
check('MK AI button is inside chart panel source structure',
  /<section class="chart-market-panel"[\s\S]*?id="chartAiMkAiBtn"/.test(source.page));
