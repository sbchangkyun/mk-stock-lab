...
check('Refresh button has title attribute (현재 포트폴리오 다시 계산)',
  ...
  portfolioContent.includes("title='현재 포트폴리오 다시 계산'"));
check('Refresh button does not claim live/real-time quote update (fixture denial allowed)',
  !portfolioContent.includes('실시간 시세 반영') &&
  !portfolioContent.includes('live quote') &&
  !portfolioContent.includes('최신 시세 반영') &&
  (!portfolioContent.includes('실시간 시세') || portfolioContent.includes('실시간 시세가 아닙니다')));
