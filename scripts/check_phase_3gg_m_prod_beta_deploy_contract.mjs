...
// This phase is authorized to modify exactly these 3 source files. The two entries below the divider
// are NOT this phase's own files -- Phase 3GG-M-PROD-HF1 (a later, separately-authorized phase) is
// allowed to modify kisClient.ts + the market-data binding for the guarded production KIS exception,
// so this checker tolerates their presence in the working tree rather than flagging them.
const ALLOWED_SOURCE_FILES = [
  ...
  // Phase 3GG-M-PROD-HF1 later-authorized modifications (documented tolerance):
  'src/lib/server/providers/kisClient.ts',
  'src/lib/server/chart-ai/local-only-live-kis-market-data-binding.mjs',
];

// Phase 3GG-M-PROD-HF1 new files, tolerated in this checker's working-tree/diff scans.
const HF1_TOLERATED_FILES = [
...
const REQUIRED_FORBIDDEN_DIFF_PATHS = [
  // kisClient.ts + local-only-live-kis-market-data-binding.mjs removed here: Phase 3GG-M-PROD-HF1 is
  // separately authorized to modify them for the scoped production KIS exception (documented tolerance).
...
const unexpectedDiff = allSourceDiffLines.filter((f) => {
  ...
  if (HF1_TOLERATED_FILES.includes(f)) return false;
