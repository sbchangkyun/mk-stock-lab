/**
 * Owner-local KIS OHLC smoke script (Phase 3ES).
 *
 * Executes a single owner-local KIS daily OHLC request and prints a SANITIZED summary only.
 * It never prints environment values, never prints request headers, and never prints or writes
 * a raw response body or actual OHLC price values. It requires three explicit owner-local smoke
 * flags to be set before any live attempt, and it writes no committed output file.
 *
...
const parseArgs = (argv) => {
  const args = { symbol: '005930', market: 'KR', assetType: 'stock', period: '1m' };
...
const flagIsExactlyTrue = (name) => {
  const raw = process.env[name];
  return typeof raw === 'string' && raw.trim().toLowerCase() === 'true';
};

const printResult = (result) => {
  process.stdout.write(`KIS owner-local OHLC smoke: ${result.status}\n`);
  process.stdout.write(`symbol: ${result.symbol}\n`);
  process.stdout.write(`market: ${result.market}\n`);
  process.stdout.write(`assetType: ${result.assetType}\n`);
  ...
  process.stdout.write(`normalizedSeriesSafe: ${result.normalizedSeriesSafe}\n`);
  process.stdout.write(`pointCount: ${result.pointCount}\n`);
  process.stdout.write(`renderable: ${result.renderable}\n`);
  process.stdout.write(`openPresent: ${result.fieldPresence.open}\n`);
  process.stdout.write(`highPresent: ${result.fieldPresence.high}\n`);
  process.stdout.write(`lowPresent: ${result.fieldPresence.low}\n`);
  process.stdout.write(`closePresent: ${result.fieldPresence.close}\n`);
  process.stdout.write(`volumePresent: ${result.fieldPresence.volume}\n`);
...
const main = async () => {
  ...
  const enableLiveQuotesFlag = flagIsExactlyTrue('KIS_ENABLE_LIVE_QUOTES');

  if (!ownerLocalFlag || !allowLiveFlag || !enableLiveQuotesFlag) {
    process.stdout.write('KIS owner-local OHLC smoke: BLOCKED\n');
    process.stdout.write(
      'reason: explicit owner-local smoke flags are not set (KIS_OWNER_LOCAL_SMOKE, KIS_ALLOW_LIVE_QUOTE, KIS_ENABLE_LIVE_QUOTES=true).\n',
    );
    process.stdout.write('rawResponsePrinted: false\n');
    process.stdout.write('secretsPrinted: false\n');
  ...
  const bundle = await build({
    stdin: {
      contents: `export { runOwnerLocalKisOhlcSmoke } from './src/lib/server/providers/kis/kisOwnerLocalOhlcClient.ts';`,
      resolveDir: root,
      sourcefile: 'kis-owner-local-ohlc-smoke-entry.ts',
      loader: 'ts',
    },
  ...
  const target = { symbol: args.symbol, market: args.market, assetType: args.assetType, period: args.period };

  const result = await mod.runOwnerLocalKisOhlcSmoke(target, context);
  printResult(result);
...
main()
  ...
  .catch((error) => {
    ...
    process.stdout.write('KIS owner-local OHLC smoke: FAIL\n');
