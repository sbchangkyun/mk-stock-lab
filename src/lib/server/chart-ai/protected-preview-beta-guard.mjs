...
// Phase 3GK stable Production access guard.
//
// Replaces the retired Phase 3GG-M-PROD-BETA-DEPLOY beta guard (evaluateProductionChartAiBetaAccess,
// which required CHART_AI_ENABLE_PRODUCTION_CHART_AI_BETA=true + ?chartAiProdBeta=1). Chart AI is now
// a stable authenticated Production product: the real Vercel Production runtime alone is eligible, no
// env flag or query opt-in required. Does not weaken or replace the Preview guard above
// (evaluateProtectedPreviewBetaAccess is unchanged) and does not touch the Supabase login gate, which
// every caller must still enforce independently. This module never reads secrets, never prints
// anything, and exposes no KIS/LLM/model/prompt/numeric data. It performs no network call and
// contacts no KIS endpoint of any kind.

// `env` carries only the coarse runtime signal the caller has already resolved (never secrets):
// { VERCEL_ENV }.
export function evaluateStableProductionChartAiAccess({ env = {} } = {}) {
  const vercelEnv = normalize(env.VERCEL_ENV);

  // Only the actual Vercel Production deployment is eligible. A non-production runtime (including
  // Preview) must always fail closed on this path -- Preview testers use the separate
  // chartAiBetaPreview=1 guard above instead.
  if (vercelEnv !== 'production') {
    return { allowed: false, reason: 'not_production_env' };
  }

  return { allowed: true, reason: 'stable_production_chart_ai_allowed' };
}
