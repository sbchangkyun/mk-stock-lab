...
- **Owner request.** Home MARKET NEWS cards whose source TITLE begins with exactly `[급보]`/
  `[긴급]`/`[속보]` (breaking) or `[단독]` (exclusive) should be visually flagged — accent border,
  static glow, explicit text badge — alongside the existing category badge, in light and dark
  mode, with no blinking/flashing animation and no color-only signaling. UX1-A's own result doc
  had explicitly deferred this exact treatment rather than treating it as in-scope.
- **New pure module `src/lib/home/homeNewsEmphasis.ts`.** `parseHomeNewsEmphasis(title)` does
  `trimStart()` + exact `startsWith()` matching against a fixed 4-entry whitelist — no regex, no
  generic bracket scan. Near-misses (`[속보성]`, `[긴급속보]`, `"오늘 [속보]..."`, `[종합]`)
  correctly yield `null` by construction, with no special-case code required.
  `deriveHomeNewsDisplayTitle` strips only a matched prefix from the visible headline (falling
  back to the original title if that would leave it blank); `HomeMarketNews.astro`'s
  `aria-label` continues to use the full, untouched original title.
- **Wiring.** `HomeMarketNews.astro`'s `renderArticles` adds an emphasis badge
  (`home-news-emphasis-badge--breaking`/`--exclusive`, text `속보`/`단독`) and a card modifier
  class (`home-news-card--breaking`/`--exclusive`) ahead of the existing category badge, which is
  unchanged and always still rendered.
- **CSS.** Six new semantic variables (`--news-breaking-accent/-glow/-badge-bg`,
  `--news-exclusive-accent/-glow/-badge-bg`) with explicit distinct values in both the `:root` and
  `body.dark-mode` blocks (this project themes via a class toggle, not
  `prefers-color-scheme`/`[data-theme]`, so both blocks needed real values). Accent uses an inset
  `box-shadow` ring (no layout shift) plus a static outer glow `box-shadow` — zero
  `animation`/`@keyframes`/`infinite`.
- **Sibling-checker reconciliation.** `check_phase_4f_ux1a_home_surface_contract.mjs` Group 6's
  scope-creep guard, which originally hard-banned the four urgency tokens anywhere in
  `HomeMarketNews.astro`, is narrowed to allow them only when routed through the reviewed
  `homeNewsEmphasis` module — preserving the anti-scope-creep intent while authorizing this
  phase's separately-reviewed introduction. `smoke:phase-4f-ux1a-home-surface` stays **25/25**;
  `check:phase-4f-ux1a-home-surface` stays **46/46**.
- **Tests.** New `smoke:phase-4f-ux1b-home-news-emphasis` — **23/23** (all required parser
  cases + rendered class/badge decision + display-title derivation). New
  `check:phase-4f-ux1b-home-news-emphasis` — **42/42** (prefix whitelist, rendering wiring,
  static-only treatment, light+dark variables, existing news contract untouched, UX1-A surface
  guard + order unchanged, no unrelated page changes, `package.json` wiring).
- **Regression.** Full Phase 4F §11 gate green end to end (UX1-A, 4A–4E, HF1, HF2,
  `mobile-baseline`, `project-lightweight-roadmap`, the 10-command Phase 4F gate); `git diff
  --check` clean; `npm ls --depth=0` clean; `npm run build` completed all real stages (known
  benign Windows post-build teardown exit code, documented in prior phases).
- **Scope.** 4 files modified (`package.json`, `check_phase_4f_ux1a_home_surface_contract.mjs`,
  `HomeMarketNews.astro`, `style.css`), 4 files new (`homeNewsEmphasis.ts`, UX1-B testsrc/smoke/
  checker scripts). No `index.astro` change, no new top-level `Home*.astro` component, no
  reordering. Does not alter `F-HIGH-02`/`PORT-10` or `F-HIGH-03`, both still IMPLEMENTED /
  PRODUCTION OWNER VERIFICATION STILL REQUIRED; Owner QA remains 0/120. PR open, not merged. See
...
- Baseline `7da540dbadaa0a5acafb9a74aec7d9fb9cfc93f8`, branch `feature/phase-4d-lab-production-completion`.
  Implemented all twelve numbered requirements from the plan's §7 (see
  `phase_4d_lab_production_completion_result_v0.1.md` §1 for the full itemized account).
- Two implementation decisions resolved open questions the plan left unbound: (1) keyboard architecture is
  scoped to the 12 legend `<button>` chips only (`aria-pressed`, native `Enter`/`Space`, existing root-scoped
  `Escape` handler) — table cells stay semantic/non-tabbable rather than absorbing 84+ new tab stops into the
  existing dense grid; (2) `lab/nps-portfolio.astro` is **not deleted** as the plan's own §7/§16 non-binding
  recommendation suggested — it is replaced with a permanent `Astro.redirect('/lab/nps-holdings', 301)`, which
  satisfies the same concern (no stale "Phase 8" shell, no reachable "OpenDART" real-provider mention) without
  breaking any existing bookmark or external link, consistent with the plan's own note that removal should not
  be silent.
- Also implemented: `scope="col"`/`scope="row"`/`<caption>` on both matrix tables with `aria-label="데이터 없음"`
  on empty cells; `tabindex="0"` + focus-visible styling on both `.lab-matrix-scroll` regions; a pure
  `exportStatusMessage` helper in `exportCardImage.ts` wired to a new `role="status" aria-live="polite"` export
  status region on `asset-class-returns.astro` and `sp500-sectors.astro` (replacing the blocking `window.alert`
  failure path); `<div>`→`<ul>`/`<li>` list semantics for the Congress/NPS preview cards; +47 lines of additive,
  Lab-scoped CSS (including `body.dark-mode`-scoped focus overrides, never a bare global override). No real
  Congress/NPS/asset-class data integration, no new provider/API/Supabase coupling, no new environment
  variable, no new dependency — consistent with the plan's non-goals.
- Nine tracked files changed (195 insertions / 69 deletions) plus three new test/tooling files
  (`scripts/smoke_phase_4d_lab_production_completion.mjs`,
  `scripts/phase_4d_lab_production_completion_testsrc.ts`,
  `scripts/check_phase_4d_lab_production_completion_contract.mjs`); full list in the result doc §2.
- New smoke suite 19/19 PASS; new static checker 62/62 PASS. All 8 pre-existing Lab-related checkers
  independently re-run and re-verified this session with **zero edits required**: `check:lab-route-split`
  104/104, `check:lab-matrix-image-export` 80/80, `check:lab-return-matrix` 114/114, `check:lab-matrix-hover`
  57/57, `check:lab-static-modules` 82/82, `check:mobile-baseline` 74/74, `check:production-domain` 33/33 all
  green; `check:mobile-ux-density-export` 67/68, the one failure a pre-existing, out-of-scope
  `LiveMarketDashboard.astro` (Phase 4B, untouched by this phase) condition confirmed by direct grep. `npm ci`
  clean (309 packages, 0 added/removed), `npm ls --depth=0` unchanged (no new dependency), `git diff --check`
  clean.
- `npm run build` (`astro build`) could not reach a local pass/fail verdict — it consistently crashes with
  Windows exit code `-1073740791` (`STATUS_STACK_BUFFER_OVERRUN`) after every build stage logs success. An
  isolation test (tracked-files-only `git stash` naming the 9 changed files explicitly, never touching the
  untracked forbidden paths) reproduced the identical crash on the unmodified baseline tree, proving this is a
  pre-existing local Windows/Node-toolchain condition, not a Phase 4D regression. Vercel's own Linux Preview
  build is the real release gate and is deferred to after the PR is opened.
- Owner authenticated visual/touch/keyboard/screen-reader QA of the Lab page group remains deferred to the
  standing Phase 4F cross-page closeout, unchanged from the plan.
- This PR must not be merged by this assistant; merge requires explicit Owner approval.
- Final classification `PHASE_4D_LAB_IMPLEMENTED_PR_READY_OWNER_MERGE_APPROVAL_REQUIRED`.
...
- **Status**: Still blocked on owner-gated Vercel project link. Classification `BLOCKED_VERCEL_PROJECT_NOT_LINKED`. The owner Vercel setup this rerun depends on is not yet done (repo still not linked; no `.vercel/project.json`). App remains deploy-ready (local regression + npm build pass); deploy not executed.
- **Baseline**: `0b9ddf2` (Phase 3GG-L-BETA-DEPLOY).
- **Branch**: rebuild/phase-1-ia-shell.
- **Goal**: Builds on Phase 3GG-L-BETA-DEPLOY. Previous deploy was blocked because Vercel project was not linked. Reruns protected Vercel Preview deploy after owner setup. Verifies Deployment Protection before live beta testing. Verifies Preview env name presence as booleans only. Runs local regression before deploy. Runs `vercel build` for Preview only. Runs `vercel deploy --prebuilt --yes` for Preview only if allowed. Does not deploy production. Does not promote to production. Does not push. Does not print Vercel env values. Does not print secrets. Does not print model names. Does not print prompt text. Does not print raw OpenAI/KIS payloads. Does not print currentPrice/volume numeric values. Does not commit `.vercel`. current_price only. H route only. No KIS endpoint expansion.
- **Result**: Vercel CLI present (54.9.1) + authenticated, but the repo is still not linked (no `.vercel/project.json`); `vercel link` still requires interactive owner project/scope selection (hard blocker #2), cascading to Deployment Protection (#3) and Preview env checks (#4). Local regression preflight all PASS and `npm run build` PASS (deploy-ready). No deploy performed; not pushed; `.vercel` not committed.
- **Next recommended phase**: Re-run Phase 3GG-L-BETA-DEPLOY-RERUN after the owner completes `vercel link` (so `.vercel/project.json` exists) + Deployment Protection + Preview env setup. Production remains prohibited.
...
- **Status**: Implemented (hotfix). Static checker and build pass; real owner LLM network smoke still pending an owner-run local dev server in this session.
- **Baseline**: `4806d29aad5e5cb6948ecd31137c12269bf8b74d` (Phase 3GG-H-FAST).
- **Purpose**: fixes the opaque `LLM_CALL_FAILED` result the owner hit while running the Phase 3GG-H-FAST owner LLM smoke (KIS healthy, `sourceStatus=ok`, but the OpenAI Responses API call or response handling failed with no further detail). Adds safe, allowlisted diagnostics so future failures reveal *why* without ever exposing credentials, the raw prompt, raw KIS payload, the actual `currentPrice` value, or the raw OpenAI response body/message text.
- **Scope**: small, targeted hotfix to `src/lib/server/chart-ai/local-only-llm-runtime-bridge.mjs` only (no refactor) — the OpenAI fetch handler now always reads the response body as text first, then branches: on a non-2xx HTTP status it classifies the failure into one of 8 safe `openAiErrorMessageClass` labels (`invalid_api_key`, `model_not_found`, `permission_denied`, `quota_or_rate_limit`, `billing_or_quota`, `server_error`, `bad_request`, `unknown_openai_error`) derived only from `httpStatus` and OpenAI's own short `error.type`/`error.code` fields (never the free-text `error.message`); on a 2xx response whose body doesn't yield extractable text, it classifies `responseShapeKind` (`output_text_present`, `output_array_text_present`, `message_content_text_present`, `no_text_output_found`, `invalid_json`, `unknown_shape`) and reports `outputTextPresent`. Also widens `extractResponseText` to additionally support the nested `output[].content[].text.value` shape and the chat-completions-style `choices[].message.content` shape, alongside the existing `output_text` and `output[].content[].text` shapes. All diagnostics are returned under a new allowlisted `diagnostics` field (`ALLOWED_LLM_DIAGNOSTICS_FIELDS`), never the raw response body or raw message text.
- **Owner smoke script updated**: `scripts/owner_smoke_phase_3gg_h_fast_local_only_llm_runtime_bridge.mjs` now prints the safe diagnostic fields (`httpStatus`, `openAiErrorType`, `openAiErrorCode`, `openAiErrorParam`, `openAiErrorMessageClass`, `responseShapeKind`, `outputTextPresent`) when `summary.ok` is false, so the owner no longer needs to re-run the same opaque smoke command repeatedly; still forbidden and still guarded: prompt text, API key, Authorization header, raw OpenAI response body, raw KIS payload, actual `currentPrice` value.
- **Checker updated**: `scripts/check_phase_3gg_h_fast_contract.mjs` gained assertions verifying the new diagnostics exports/functions exist, that the bridge never derives `openAiErrorMessageClass` from a raw error message, that the smoke script never reads raw OpenAI `error.message`/`type`/`code`/`param` fields directly, that the smoke script's existing forbidden-raw-response pattern does not false-positive on the new `responseShapeKind` label, and that the new HF1 result doc is present with all required sections.
- **Endpoint used**: `current_price` only, unchanged from Phase 3GG-E-INTEGRATE / 3GG-F-FAST / 3GG-G-FAST / 3GG-H-FAST.
- **Activation status**: no public activation; no beta activation; no internal QA activation; no UI wiring added in this hotfix.
- **Preserved policy**: no order endpoint; no cancel/modify order endpoint; no account endpoint; no balance endpoint; no funds endpoint; no buying power endpoint; no sellable quantity endpoint; no profit/loss endpoint; no deposit/withdrawal endpoint; no trading history endpoint; no portfolio/holdings endpoint; no personal endpoint; no raw KIS payload exposure; no credential exposure; no raw LLM response/message exposure; no MK Agent/Similar Pattern auto-run; no new dependency; no lockfile change; no Supabase change; no existing KIS provider module change; `.env`/`.env.local` not touched; not pushed; not deployed.
- **Recommended next step**: obtain the real owner LLM smoke run against a local dev server (owner action) to confirm the specific `openAiErrorMessageClass`/`responseShapeKind`, then proceed to Phase 3GG-I-FAST - Chart AI UI KIS + LLM Summary Wiring once the LLM call itself is confirmed working end-to-end.
...
- **Status**: Recorded.
- **Baseline**: `ab44382a623d17243082af8cf899719789a98742` (Phase 3GG-B-AUDIT).
- **Purpose**: record the owner's review decisions for all 11 Live KIS approval gates after Phase 3GG-B-AUDIT.
- **Scope**: 22-section owner review record document (`docs/planning/phase_3gg_b_review_record_live_kis_owner_review_v0.1.md`) recording the owner review decision for Gate 1 through Gate 11; the Gate 2 expanded market-data endpoint allowance required for the MK AI analysis goal (both the previously "initial allowed" and "separate review recommended" market-data endpoint categories are approved together); confirmation that forbidden account/trading/order/balance/personal endpoints remain blocked; an activation readiness assessment; a result document; a static contract checker (`scripts/check_phase_3gg_b_review_record_contract.mjs`); one new `check:phase-3gg-b-review-record` package script. All 11 gates now carry a recorded owner decision (Approved or Approved with condition). No gate is rejected. Live KIS is not activated by this phase.
- **Preserved policy**: no source change; no `chart-ai.astro` change; no API route created or activated; no scaffold/agent/provider source or fixture change; no live KIS; no LLM; no MK AI route activation; no Supabase/DB real runtime; no env/session/JWT/cookie/header parsing; no public/beta/internal QA activation; no usage deduction; no paid entitlement; no ad unlock; no dependency/lockfile change; no deploy; no push.
- **Recommended next step**: Phase 3GG-C - Live KIS Activation Decision Record, No Activation.
...
- **Status**: Implemented.
- **Purpose**: Correct reviewer-name encoding in the owner visual QA evidence documents.
- **Baseline**: `907f959`.
- **Phase 3FE-A evidence reference**: `## Phase 3FE-A - 2026-07-07`; `### KIS OHLC Provider Owner-local Integration (Implemented)`; fixture-only KIS OHLC provider boundary; owner-local Similar Pattern route; No live KIS call; no environment read; no database.
- **Defect**: reviewer name was recorded with mojibake/encoding corruption; correct reviewer is the UTF-8 string generated by `String.fromCodePoint(0xAE40, 0xCC3D, 0xADE0)`; expected UTF-8 hex is `eab980ecb0bdeab7a0`.
- **Implemented scope**: corrected evidence result reviewer name, corrected closeout result reviewer name, updated evidence checker to assert the codepoint-generated reviewer name, added reviewer hex validation, added reviewer-line corruption guards, added HF1 result document, added HF1 checker, updated package script, and updated changelog.
- **Preserved policy**: no runtime/source/API/UI/provider changes, no live KIS, no LLM, no MK AI route activation, no Supabase/DB/env/session/JWT, no public/beta activation, no dependency/lockfile change, no deploy/push.
- **Recommended next step**: `Phase 3FF-A-PLAN` only as planning-only.
- **Hold**: live KIS, beta activation, public activation, direct Phase 3FF-A implementation, deploy, and push remain blocked.
...
- **Correction note**: evidence reviewer encoding corrected by `Phase 3FE-A-MANUAL-QA-RUN-CLOSEOUT-EVIDENCE-HF1`.
- **Preserved policy**: no runtime/source/API/UI/provider changes, no live KIS, no LLM, no MK AI route activation, no Supabase/DB/env/session/JWT, no public/beta activation, no dependency/lockfile change, no deploy/push.
- **Recommended next step**: `Phase 3FF-A-PLAN` only as planning-only.
...
- **Status**: Implemented.
- **Purpose**: Resolve the stale checker-scope blocker that prevented the closeout package from being committed, and reduce recurrence of the same checker-scope failure across the Phase 3FE-A manual-QA validation chain.
- **Baseline**: `a191dfc`.
...
- **Implemented scope**: preserved closeout checklist, preserved closeout result document, preserved closeout checker, corrected stale checker scope in affected validation-chain checkers, added HF1 result document, added HF1 checker, updated package script, updated changelog.
- **Root cause**: older checker scope rejected legitimate post-phase retry files.
- **Anti-recurrence measure**: validation-chain checkers now separate original phase diff validation from current forbidden runtime/source/dependency drift checks.
- **Preserved policy**: no runtime/source/API/UI/provider changes, no live KIS, no LLM, no MK AI route activation, no Supabase/DB/env/session/JWT, no public/beta activation, no dependency/lockfile change, no deploy/push.
- **Recommended next step**: owner execution of the visual checklist, or closeout evidence update after owner evidence is provided.
...
- **Status**: Implemented. Completed the Phase 3FE-A evidence metadata by recording commit hash `1b2a0f2` in the Phase 3FE-A result evidence and changelog context.
- **Purpose**: Resolve the Phase 3FE-A evidence-lock HOLD by making the anti-hallucination evidence explicit: phase name, commit hash, status, and validation result.
- **Scope**: Documentation-only update to `docs/planning/phase_3fe_a_kis_ohlc_provider_owner_local_integration_result_v0.1.md` and `docs/planning/planning_changelog.md`.
- **Preserved policy**: no runtime source change, no API route change, no UI change, no provider/helper source change, no checker or smoke script change, no package or lockfile change, no dependency change, no live KIS, no LLM, no MK AI route activation, no Supabase client, no database connection, no environment credential read, no cookie/header/session/JWT parsing, no public/beta activation, no deploy, no push.
- **Recommended next step**: Rerun Phase 3FE-A Evidence Lock and proceed to Phase 3FE-A-HANDOFF only if it returns PASS. Hold: Phase 3FF-A remains blocked until the evidence lock and handoff steps are complete.
...
- **Evidence metadata**: Commit `1b2a0f2`; status Implemented; Phase 3FE-A checker PASS (`188/188`); Phase 3FE-A smoke PASS (`141/141`); Phase 3FD-J and Phase 3FD-I regression checks/smokes passed; build passed; diff check passed.
...
- **Status**: Prepared. Added a documentation-only design plan for making `/chart-ai` a login-required page aligned with the Portfolio access pattern and for adding a master-user cooldown exemption in a future mocked-only UI implementation. No UI source file, route source file, server runtime source file, auth implementation, database connection, Supabase client, environment read, cookie/header/session/JWT parsing, migration execution, API/LLM call, live KIS call, package install, dependency change, deploy, or push occurred.
- **Background**: Phase 3FD-G-HF1 added a 5-minute client-side cooldown after analysis success. Owner browser review confirmed the countdown direction and then requested that Chart AI become a login-required page like Portfolio, while a master account should bypass cooldown. Raw master account identifiers are sensitive and must not be committed or exposed in client-side code.
- **Implemented scope**: consolidated login-gate and master cooldown exemption design plan, sensitive identifier policy, mocked-only next implementation option, real-auth future option requiring separate approval, server-side protection requirement, result doc, narrow static checker, package script, changelog.
- **Preserved policy**: no `/chart-ai` source change, no Portfolio source change, no route source change, no server runtime source change, no provider source change, no deterministic engine source change, no data source change, no auth runtime implementation, no database connection, no Supabase client creation, no environment value read, no cookie/header/session/JWT parsing, no migration execution, no API/LLM call, no live KIS call, no actual server-side usage limiting, no persistence, no payment/ad integration, no package install, no dependency or lockfile change, no raw master identifiers committed, no route success, no beta/public activation, no deploy, no push.
- **Recommended next phase**: Phase 3FD-H — Chart AI Login Gate and Master Cooldown Exemption Mocked-only UI Implementation. Alternative: Phase 3FD-H-HF1 — Login Gate/Master Exemption Design Revisions, No Runtime Change. Hold alternative: Phase 3FD-B-HF1 — Real Supabase Client Factory Approval Package, No Runtime Change.
...
- **Status**: Prepared. Added a lean documentation-only manual QA and activation boundary review for the all-gates-off guarded route composition scaffold. No route source file, UI file, server runtime source file, database connection, Supabase client, environment read, migration execution, package install, dependency change, deploy, or push occurred.
- **Background**: Phase 3FD-E added the guarded route runtime composition scaffold inside the existing guarded branch while preserving safe blocked behavior. This phase defines owner-local manual QA scenarios and activation approval gates before any route success or provider execution can be considered.
- **Implemented scope**: consolidated owner-local manual QA and activation boundary plan, result doc, narrow static checker, package script, changelog.
...
- **Recommended next phase**: Phase 3FD-F — Owner-local Guarded Composition Manual QA Package, No Runtime Change. Alternative: Phase 3FD-E-HF1 — Guarded Composition Scaffold Revisions, All Gates Off. Hold alternative: Phase 3FD-B-HF1 — Real Supabase Client Factory Approval Package, No Runtime Change.
...
- **Status**: Prepared. Added an owner-local mocked smoke and static checker verifying the Phase
  3FC-H guarded route scaffold branch through the real route handlers. No runtime source file was
  changed; the guarded branch still always falls back to the existing feature-disabled shell.
- **Background**: Phase 3FC-H added the guarded-runtime-scaffold branch to the Chart Similarity API
  route. This phase verifies that branch's behavior with a dedicated owner-local smoke, without
  opening public/beta activation and without a live KIS call.
- **Implemented scope**: owner-local mocked guarded route smoke (110 assertions), static contract
  checker, scenarios doc, result doc, package scripts, changelog.
- **Preserved policy**: no live KIS call, no KIS network diagnostics, no `.env`/env/credential/token
  inspection or printing, no Vercel env read/change, no deploy, no push, no public live route
  success, no public KIS data exposure, no account/trading/order/balance APIs, no Supabase package
  import or real Supabase client, no cookie/header/session parsing, no feature flag DB/cache
  runtime, no usage DB/cache runtime, no role DB runtime, no SQL/migration files, no new
  dependencies, no KIS provider/deterministic engine/API route/`/chart-ai`/guarded route scaffold
  runtime source change, no data fixture change, no external AI, no dev server start, no manual
  browser QA.
- **Recommended next phase**: Phase 3FC-J — Guarded Route Runtime Composition Plan Refresh, No
  Runtime Change. Alternative: Phase 3FD-A — Real Supabase/Auth/Usage Implementation Approval
  Package, No Runtime Change.
...
- **Status**: Implemented. `POST /api/chart-ai/similarity` now has a second explicit, owner-local-only branch that evaluates the existing `evaluateSimilarityExecutionGuard` against a caller-supplied mock auth/usage request before allowing the Phase 3FB-A mocked provider-compatible integration to run. Default route/UI behavior and the existing Phase 3FB-B owner-local-mocked branch are unchanged. Live KIS remains disabled and was never called.
- **Background**: Phase 3FB-D hardened the owner-local mocked UI runtime, but the API route itself still had no auth/usage evaluation in front of mocked execution. This phase connects the already-designed `evaluateSimilarityExecutionGuard` (Phase 3EY-C) to a real server-side route execution flow for the first time, using caller-supplied mock auth/usage state rather than a real session or usage store — not another planning-only, closeout-only, network-check, credential-check, or harness-only phase.
- **Implemented scope**: new `similarityAuthUsageRouteBridgeTypes.ts`, `similarityAuthUsageRouteBridge.ts` (`buildDefaultSimilarityAuthUsageRouteBridgePolicy`, `isOwnerLocalAuthUsageBridgeRequestBody`, `normalizeSimilarityAuthUsageBridgeRequestBody`, `mapBridgeRequestToSimilarityExecutionGuardRequest`, `mapBridgeUsageToGuardUsageSnapshot`, `runSimilarityAuthUsageRouteBridge`), and `mockedSimilarityAuthUsageRouteBridgeFixtures.ts`; `similarityApiResponseTypes.ts` gained `'owner-local-auth-usage-bridge'` mode and `SimilarityApiAuthUsageBridgeSuccessData`; `similarityApiResponseBuilder.ts` gained `isOwnerLocalAuthUsageBridgeSimilarityApiRequestBody`, `mapAuthUsageBridgeApiStatusToHttpStatus`, `buildOwnerLocalAuthUsageBridgeSimilarityApiResponse`; `similarity.ts` route gained one additional mutually exclusive branch; `index.ts` exports updated with no removals; new 59-assertion smoke script and 76-assertion static contract checker.
- **Route result**: new request shape `{ mode: "owner-local-auth-usage-bridge", source: "mocked-provider-compatible", ownerLocalAuthUsageBridge: true, symbol?, assetType?, mockAuth: { state, role }, mockUsage: { window, used, limit, remaining } }` maps guard status to HTTP status (`success`→200, `auth_required`→401, `usage_limited`→429, `blocked`/`not_configured`/`error`→422); owner/admin role with safe usage reaches guard-allowed mocked execution and a sanitized bucketed response; anonymous/usage-at-limit/internally-inconsistent requests are blocked safely with no raw error; default and existing owner-local-mocked branches are unchanged.
- **Preserved policy**: no live KIS call, no `fetch`, no `process.env`/`.env` read, no KIS provider source change, no deterministic engine source change, no `/chart-ai` UI change, no real external auth provider import, no real usage DB/cache/SQL/migration, no account/trading/order/balance fields, no public/live execution enabled (`allowPublicKisExecution`/`allowLiveKis` fixed false), no dependency/lockfile change, no deployment, no push.
- **Recommended next phase**: Phase 3FB-E — wire the owner-local auth/usage bridge into a local-only, explicit opt-in `/chart-ai` panel (mirroring the Phase 3FB-C/3FB-D gating pattern). Alternative: Phase 3FB-F — Chart AI Owner-local Auth/Usage Bridge Manual QA and Productization Boundary Review, Live KIS Off.
...
- **Status**: Implemented and executed. This is a credential configuration check only — it is not a KIS call phase and not a retry of the manual smoke.
- **Background**: the prior Phase 3FA-D-MANUAL-RUN owner-approved manual smoke attempt resolved to `failed_redacted` because required KIS credential env names were not present in that session; this phase adds a redacted, no-secret-echo check to determine required env key name presence in advance of any future manual-run retry.
- **Implemented scope**: added `similarityOwnerLocalCredentialCheckTypes.ts` (status/decision/source/key-requirement/key-status/policy/report/result types), `similarityOwnerLocalCredentialCheck.ts` (`buildDefaultSimilarityOwnerLocalCredentialCheckPolicy`, `buildOwnerLocalCredentialKeyRequirements`, `buildOwnerLocalCredentialKeyStatuses`, `buildOwnerLocalCredentialCheckReport`, `buildOwnerLocalCredentialCheckResult`, `assertCredentialCheckReportHasNoSecretEcho`) which defines a local, safe copy of the required KIS credential env key names (`KIS_APP_KEY`, `KIS_APP_SECRET`, `KIS_BASE_URL`) mirroring the existing owner-local KIS OHLC client without importing it, `mockedSimilarityOwnerLocalCredentialCheckFixtures.ts` (deterministic non-live fixtures), updated `src/lib/server/chartSimilarity/index.ts` exports, a credential-check smoke script (`scripts/smoke_phase_3fa_d_manual_run_hf1_kis_credential_configuration_check.mjs`) — the only place permitted to read `process.env`, presence-boolean only — and a new static checker with matching `package.json` script entries.
- **Credential check result**: resolved to status `missing`, decision `missing_required_env`, `readyForManualRunRetry: false`. All three required KIS credential env key names — `KIS_APP_KEY`, `KIS_APP_SECRET`, `KIS_BASE_URL` — were reported not present in this session. `valueEchoed`, `dotenvRead`, `kisCallAttempted`, and `routeCallAttempted` were all `false`. No actual environment value, value length, prefix/suffix, or hash was read, printed, or persisted anywhere.
- **Preserved policy**: no API route change, no `/chart-ai` UI change, no change to the deterministic similarity engine, the existing owner-local KIS OHLC client, or the Phase 3FA-D-MANUAL-RUN module/fixtures/script, no KIS call, no route call, no public/beta execution, no real auth runtime, no DB/cache runtime, no SQL/migration, no external AI, no `source=live`, no `source=auto`, no account/trading/order/balance APIs, no Vercel env changes, no deployment, no push, no dependency changes, no `.env` file read.
- **Validation**: the new Phase 3FA-D-MANUAL-RUN-HF1 static checker passed; the new smoke script printed only a redacted JSON report (exit `0`); followed by the Phase 3FA-D-MANUAL-RUN checker, the Phase 3FA-D closeout checker/smoke, the full established historical checker/smoke suite, `npm run build`, and `git diff --check`; see the final phase report for the itemized pass/fail list, including expected non-gating failures in stale historical phase checkers.
- **Recommended next step**: because this check resolved to `missing`, the recommended next step is to configure the three missing KIS credential env key names in the owner's own local shell session, then re-run this HF1 check; alternative — proceed to Phase 3FB-A without a KIS success confirmation. No manual-run retry is approved by this phase.

## Phase 3FA-D-MANUAL-RUN - 2026-07-04

### Owner-local Manual Smoke Execution, Separately Approved (Implemented)

- **Status**: Implemented and executed — one owner-approved owner-local KIS OHLC manual smoke attempt made; redacted result recorded.
- **Approval scope**: the owner explicitly approved a KIS call for this phase only ("Phase 3FA-D-MANUAL-RUN 진행, (KIS 호출 승인)"); this approval does not extend to public execution, beta execution, route success, API route integration, UI integration, DB/cache storage, auth runtime, deployment, or push.
- **Implemented scope**: added `similarityOwnerLocalManualRunTypes.ts` (manual-run status/decision/check-status/check/policy/provider-probe/engine-contract-check/redaction-check/report/result types), `similarityOwnerLocalManualRun.ts` (`buildDefaultSimilarityOwnerLocalManualRunPolicy`, `buildApprovedSimilarityOwnerLocalManualRunPolicy`, `bucketNormalizedBarCount`, `buildOwnerLocalManualRunPreflightChecks`, `buildOwnerLocalManualRunBlockedReport`, `buildOwnerLocalManualRunRedactedReport`, `buildOwnerLocalManualRunResult`, `sanitizeManualRunSerializedOutput`, `assertManualRunReportIsRedacted`), `mockedSimilarityOwnerLocalManualRunFixtures.ts` (deterministic non-live fixtures), updated `src/lib/server/chartSimilarity/index.ts` exports, a manual-run script (`scripts/smoke_phase_3fa_d_manual_run_owner_local_manual_smoke_execution.mjs`) that reuses the existing, already-approved owner-local KIS OHLC client (`runOwnerLocalKisOhlcSmoke`, Phase 3ES) instead of creating a new client, and a new static checker (165 checks) with matching `package.json` script entries.
- **Manual run result**: redacted status `failed_redacted`, decision `failed_before_provider_call` — the provider probe reported `blocked` with `normalizedBarsAvailable: false` and bucket `none`, safe message "Required KIS credential env names are not present in this session (3 missing); live smoke blocked."; the engine contract check was `not_run` by design; the redaction check passed with all leak booleans `false`; all eleven preflight checks resolved `pass`; no raw KIS payload, actual OHLC value, credential, token, or environment value was printed or persisted.
- **Preserved policy**: no API route change (`src/pages/api/chart-ai/similarity.ts` untouched), no `/chart-ai` UI change, no change to the deterministic similarity engine or the existing owner-local KIS OHLC client, route success remains disabled, no public/beta execution, no real auth runtime, no usage storage implementation, no DB/cache runtime, no SQL/migration, no external AI, no `source=live`, no `source=auto`, no account/trading/order/balance APIs, no Vercel env changes, no deployment, no push, no dependency changes, no `.env`/`process.env` value printed or committed.
- **Validation**: the new Phase 3FA-D-MANUAL-RUN static checker (165 checks) passed; the manual-run script was run both without approval flags (safe blocked report) and with both approval flags (the one approved manual run); followed by the Phase 3FA-D closeout checker/smoke and the full established historical checker/smoke suite, `npm run build`, and `git diff --check`; see the final phase report for the itemized pass/fail list, including expected non-gating failures in stale historical phase checkers.
- **Recommended next phase**: Phase 3FB-A, to decide the next scoped step based on this manual run's redacted outcome. No route, UI, auth, storage, or deployment work is approved by this phase.

## Phase 3FA-D - 2026-07-04

### Owner-local Manual Smoke Execution Closeout (Prepared/Implemented)

- **Status**: Prepared/Implemented — owner-local manual smoke execution closeout added, no live smoke executed.
- **Background**: Phase 3FA-C added a disabled-by-default owner-local KIS similarity smoke harness scaffold. This phase closes out that disabled harness/readiness layer without executing any owner-local KIS smoke: it defines the manual closeout checklist, owner-approval gates, safe execution decision record, redacted closeout report shape, and next-step decision model.
- **Implemented scope**: added `similarityOwnerLocalSmokeCloseoutTypes.ts` (closeout status/decision/check-status/check/policy/report/result types), `similarityOwnerLocalSmokeCloseout.ts` (`buildDefaultSimilarityOwnerLocalSmokeCloseoutPolicy`, `buildOwnerLocalSmokeCloseoutChecks`, `buildOwnerLocalSmokeCloseoutReport`, `buildSimilarityOwnerLocalSmokeCloseoutResult`, `isOwnerLocalManualSmokeExecutionClosed`, `isOwnerLocalManualSmokeReadyForSeparateApproval`), `mockedSimilarityOwnerLocalSmokeCloseoutFixtures.ts` (deterministic fixtures), updated `src/lib/server/chartSimilarity/index.ts` exports, a committed no-live-runtime smoke script (38 assertions against the actual closeout module), and a new static checker (127 checks) with matching `package.json` script entries.
- **Closeout results**: default policy `enabled: false`, `liveSmokeExecuted: false`, all execution permission flags (`allowLiveKisCall`, `allowLiveSimilarityExecution`, `allowRouteSuccess`, `allowRouteCall`, `allowEnvRead`, `allowMarketDataInReport`, `allowRawProviderPayload`, `allowCredentialEcho`) `false`, `requireOwnerApprovalForNextPhase: true`, `requireManualSmokeSeparateCommand: true`; twelve closeout checks, ten resolving `pass`/`not_run` and two (`owner_approval_required_for_next_phase`, `manual_smoke_requires_separate_command`) resolving `blocked`; `isOwnerLocalManualSmokeExecutionClosed(defaultPolicy)` is `true`; `isOwnerLocalManualSmokeReadyForSeparateApproval(defaultPolicy)` is `true`; closeout report `status: 'closed_without_execution'`, `decision: 'request_owner_approval_for_manual_smoke'`, `smokeExecuted: false`, `harnessStatus: 'disabled'`, `routeStatus: 'feature_disabled'`, `nextAllowedPhase: '3FA-D-MANUAL-RUN'`.
- **Preserved policy**: no KIS call, no live similarity execution, no route call, no import of the real similarity engine or any live KIS provider/client, no API route change (`src/pages/api/chart-ai/similarity.ts` untouched), no `/chart-ai` UI change, no real auth runtime, no usage storage implementation, no DB/cache runtime, no SQL/migration, no external AI, no `source=live`, no `source=auto`, no account/trading/order/balance APIs, no Vercel env changes, no deployment, no push, no dependency changes, no actual market values, no `.env`/`process.env` read by the closeout module or its smoke script; the Phase 3FA-C disabled harness behavior is preserved unchanged.
- **Validation**: the new Phase 3FA-D static checker (127 checks) and committed smoke script (38 assertions) both passed, followed by the full established checker/smoke suite (Phase 3FA-C + smoke, 3EX-E-HF1, 3FA-B, 3FA-A, 3EZ-C, 3EZ-B, 3EZ-A, 3EX-E, 3EY-D + smoke, 3EY-C + smoke, 3EX-C smoke, 3EW-C, 3EW-B, 3EW-A, 3EV-B, 3EV-A, `check:provider-boundaries`, `check:kis-runtime-guard`, `check:kis-error-fallback`, `check:production-domain`), `npm run build`, and `git diff --check`; see the final phase report for the itemized pass/fail list, including expected non-gating failures in stale historical phase checkers.
- **Recommended next phase**: Phase 3FA-D-MANUAL-RUN — a separately authorized, owner-approved phase to execute an owner-local manual smoke. Alternative: Phase 3FB-A.

## Phase 3FA-C - 2026-07-04

### Owner-local KIS Similarity Smoke Harness, Disabled by Default (Implemented)

- **Status**: Implemented — disabled owner-local KIS similarity smoke harness scaffold added, no live smoke executed.
- **Background**: Phase 3FA-B defined the owner-local KIS similarity smoke plan (design-only, no smoke executed). This phase adds a harness scaffold layer on top of that plan: a disabled-by-default runtime boundary with ordered preflight steps, static safety checks, and a deterministic blocked report, ready for a future, separately authorized phase to extend for actual owner-local manual smoke execution.
- **Implemented scope**: added `similarityOwnerLocalSmokeHarnessTypes.ts` (harness status/mode/step/check/policy/report/result types), `similarityOwnerLocalSmokeHarness.ts` (`buildDefaultSimilarityOwnerLocalSmokeHarnessPolicy`, `buildOwnerLocalSmokeHarnessSteps`, `buildOwnerLocalSmokeHarnessChecks`, `buildOwnerLocalSmokeHarnessBlockedReport`, `runOwnerLocalSmokeHarnessDisabled`, `isOwnerLocalSmokeHarnessEnabled`), `mockedSimilarityOwnerLocalSmokeHarnessFixtures.ts` (deterministic fixtures), updated `src/lib/server/chartSimilarity/index.ts` exports, a committed smoke script (38 assertions against the actual harness module), and a new static checker (132 checks) with matching `package.json` script entries.
- **Harness results**: default policy `enabled: false`, `mode: 'disabled_harness'`, `ownerLocalOnly: true`, `manualExecutionOnly: true`, all execution permission flags (`allowKisProviderCall`, `allowSimilarityEngineRun`, `allowRouteSuccess`, `allowMarketDataInReport`, `allowRawProviderPayload`, `allowEnvRead`, `allowCredentialEcho`) `false`, `requireOwnerApprovalBeforeLiveSmoke: true`, `requireSeparateHarnessEnableApproval: true`; ordered steps `load_smoke_plan` → `check_harness_policy` → `verify_route_remains_disabled` → `verify_redaction_policy` → `verify_no_live_provider` → `verify_no_live_engine` → `build_safe_blocked_report`; nine static safety checks report `pass` and one (`separate_owner_approval_required`) reports `blocked`; `isOwnerLocalSmokeHarnessEnabled(defaultPolicy)` is `false`; `runOwnerLocalSmokeHarnessDisabled()` top-level `status` is `'disabled'`, never `'pass'`/success.
- **Preserved policy**: no KIS call, no live similarity execution, no import of the real similarity engine or any live KIS provider/client, no API route change (`src/pages/api/chart-ai/similarity.ts` untouched), no `/chart-ai` UI change, no real auth runtime, no usage storage implementation, no DB/cache runtime, no SQL/migration, no external AI, no `source=live`, no `source=auto`, no account/trading/order/balance APIs, no Vercel env changes, no deployment, no push, no dependency changes, no actual market values, no `.env`/`process.env` read by the harness or its smoke script.
- **Validation**: the new Phase 3FA-C static checker (132 checks) and committed smoke script (38 assertions) both passed, followed by the full established checker/smoke suite (Phase 3EX-E-HF1, 3FA-B, 3FA-A, 3EZ-C, 3EZ-B, 3EZ-A, 3EX-E, 3EY-D + smoke, 3EY-C + smoke, 3EX-C smoke, 3EW-C, 3EW-B, 3EW-A, 3EV-B, 3EV-A, `check:provider-boundaries`, `check:kis-runtime-guard`, `check:kis-error-fallback`, `check:production-domain`), `npm run build`, and `git diff --check`; see the final phase report for the itemized pass/fail list, including expected non-gating failures in stale historical phase checkers.
- **Recommended next phase**: Phase 3FA-D — Owner-local Manual Smoke Execution Closeout. Alternative: Phase 3EX-E-OWNER-RUNTIME-CHECK.

## Phase 3EX-E-HF1 - 2026-07-04

### Analysis Workspace Tagged Page Swap UI Fix (Implemented)

- **Status**: Implemented — `/chart-ai` analysis workspace now behaves as a real tagged page-swap interface; no backend/runtime/route change.
- **Background**: Owner feedback on the polished Phase 3EX-E analysis workspace was that the "유사 패턴 분석 보기" and "MK AI 분석 보기" controls visually toggled active state, but the content below still read as one continuous long page instead of two distinct pages. This phase is a pure frontend/UI hotfix to `src/pages/chart-ai.astro` fixing the post-it/page-swap interaction and confirming true content separation.
- **Implemented scope**: restructured the workspace into a `chart-analysis-book` wrapper containing a `chart-analysis-tags` tab strip (`role="tablist"`, two `role="tab"` buttons, `aria-selected`/`aria-controls`) above a single `chart-analysis-page-canvas`, which now holds the two `chart-analysis-page` (`role="tabpanel"`) sections, `chartAiSimilarityPanel` (active/visible by default) and `chartAiMkAiPanel` (`hidden` by default); updated the client-side tab script to toggle `active`/`aria-selected`/`tabindex`/`hidden` together on click and to support `ArrowLeft`/`ArrowRight`/`Home`/`End` keyboard navigation with roving focus; added post-it/page-canvas CSS (`.chart-analysis-tag`, `.chart-analysis-tag.active`, `.chart-analysis-page-canvas`, `.chart-analysis-page[hidden] { display: none !important; }`, a sub-180ms `chart-analysis-page-swap-enter` keyframe fade, `prefers-reduced-motion` handling, and a mobile stacked-tag layout); added the Phase 3EX-E-HF1 result doc, static checker, and package script.
- **UI behavior result**: only one page is visible at a time; clicking a tag swaps the entire content canvas as if bringing a distinct page to the front, with the active tag visually raised/attached to the page canvas and the inactive tag visually behind/softer; the swap uses a small fade/translate under 180ms and is disabled entirely under `prefers-reduced-motion`; no scroll-into-inactive-page and no layout jank.
- **Content separation result**: confirmed the similarity page contains only similarity content (유사 패턴 분석 결과, 현재 패턴 요약, 유사 구간 Top 5, 기준일 100 정규화 오버레이, 사후 성과 요약, 데이터 한계 / 투자 유의, similarity disclaimers) and the MK AI page contains only MK AI content (MK AI 해석, AI 분석 미리보기, 요약, 핵심 해석, 시나리오 점검, 분석 근거, 확인 체크리스트, 리스크 체크리스트, 데이터 한계, MK AI disclaimers), with no Top 5 table or overlay chart inside the MK AI page and no nested MK AI content inside the similarity page.
- **Preserved policy**: no API route change, no KIS call, no live similarity execution, no real auth runtime, no usage storage implementation, no DB/cache runtime, no SQL/migration, no external AI, no `source=live`, no `source=auto`, no account/trading/order/balance APIs, no Vercel env changes, no deployment, no push, no dependency changes, no actual market values, no `.env`/`process.env` read; search behavior, sample chart behavior, selected-symbol overview, KIS preview buttons/copy, and all sample/mock/investment/KIS-not-real/no-external-AI disclaimers and the Phase 3EX-D/E chart/sidebar stretch fix remain intact.
- **Validation**: the new Phase 3EX-E-HF1 static checker and the full established checker/smoke suite were run, plus `npm run build` and `git diff --check`; see the final phase report for the itemized pass/fail list, including the expected non-gating failure in the now-superseded Phase 3EX-E checker due to the renamed `chart-analysis-tag`/`chart-analysis-page` classes.
- **Recommended next phase**: Phase 3FA-C — Owner-local KIS Similarity Smoke Harness, Disabled by Default. Alternative: Phase 3EX-E-HF2.
